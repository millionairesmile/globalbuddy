...
function addMenuToList(name, menu, key) {
  ...
  orderSpan.textContent = ""; // 순번은 추후 업데이트

  const textSpan = document.createElement("span");
  textSpan.textContent = `${name}: ${menu}`;

  listItem.appendChild(orderSpan); // 순번 추가
  listItem.appendChild(textSpan);
...
function deleteMenu(menuKey) {
  remove(dbRef(db, "menus/" + menuKey))
    .then(() => {
      console.log("메뉴가 삭제되었습니다.");
      updateMenuListOrder(); // 삭제 후 순번 재정렬
    })
    .catch((error) => {
      console.error("메뉴 삭제 중 오류 발생:", error);
      alert("메뉴 삭제 중 오류가 발생했습니다.");
...
onValue(menusRef, (snapshot) => {
  ...
  if (snapshot.exists()) {
    ...
    updateMenuListOrder(); // 메뉴 목록 업데이트 후 순번 재정렬
  }
});
